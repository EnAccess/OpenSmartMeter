...
void urgeent(){
   ...
   if ( ((freq > low_freq) || (mains_input_value < over_voltage) || (mains_input_value > low_voltage) || (true_power < over_load)) && (btt > 2) ) {
   fault = 0;
   }
   }
}
...
void credit_formular() {
  ...
  if (true_power > 0) {
  ENERGY  = ENERGY  +  ( ( true_power) / (  3600000 ) ); // energy per KWH(energy gotten per sec)
...
void check_tokenused() {
   ...
   for(locationcount=40;  locationcount<token_eeprom_location; locationcount++ ){
...
void STStoken_decode() {
   ...
   if ((confirmkey == private_stskey) && ( convertedsts_data != eeprom_sts_data ) && (trueMT_NO == meter_no) && (meter_no_count == stsnew_meter_no_count) ) {
