   ...
   unsigned short ATM90E26_UART::AFECHIP(unsigned char RW,unsigned char address, unsigned short val){
   ...
   short int apower= (short int)AFECHIP(1,PmeanTwo,0xFFFF); //Complement, MSB is signed bit
   ...
   return (double)Aenergy*0.0001; //returns kWh if PL constant set to 1000imp/kWh
   }
...
void ATM90E26_UART::SET_register_values(){
   ...
   AFECHIP(0,  SoftReset,  0x789A); //Perform soft reset
   AFECHIP(0,  FuncEn,     0x0030); //Voltage sag irq=1, report on warnout pin=1, energy dir change irq=0
   AFECHIP(0,  SagTh,      0x1F2F); //Voltage sag threshhold
		

   //Set metering calibration values
   AFECHIP(0,  CalStart,   0x5678); //Metering calibration startup command. Register 21 to 2B need to be set
   AFECHIP(0,  PLconstH,   0x00B9); //PL Constant MSB
   AFECHIP(0,  PLconstL,   0xC1F3); //PL Constant LSB
   AFECHIP(0,  Lgain,      0x0000); 	//Line calibration gain
   AFECHIP(0,  Ngain,      0x0000); 	//Neutrl calibration gain
   AFECHIP(0,  Lphi,       0x0000); //Line calibration angle
   AFECHIP(0,  PStartTh,   0x08BD); //Active Startup Power Threshold
   AFECHIP(0,  PNolTh,     0x0000); //Active No-Load Power Threshold
   AFECHIP(0,  QStartTh,   0x0AEC); //Reactive Startup Power Threshold
   AFECHIP(0,  QNolTh,     0x0000); //Reactive No-Load Power Threshold
   AFECHIP(0,  MMode,      0x9222); //9022 Metering Mode Configuration. All defaults. See pg 31 of datasheet.
   AFECHIP(0,  CSOne,      0x4A34); //Write CSOne, as self calculated
   ...
   AFECHIP(0,  Ugain,       0xD464);    //Voltage rms gain
   AFECHIP(0,  IgainL,      0x6E49);   //L line current gain
   //AFECHIP(0,  IgainN,     0x6E49);   // N line current gain
   ...
   AFECHIP(0,  CSTwo,       0xD294); //Write CSTwo, as self calculated
